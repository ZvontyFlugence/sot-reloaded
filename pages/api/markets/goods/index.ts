...
export default async (req: NextApiRequest, res: NextApiResponse) => {
	switch (req.method) {
		case 'GET': {
			...
			let productOffers = await withPrisma(async (client: PrismaClient) => {
				// Dont need transaction b/c country currency cant change (for now)
				const country = await client.country.findUnique({
					where: { id: countryId },
					select: { currency: { select: { code: true } } },
				})

				return await client.productOffer.findMany({
					where: {
			...
			productOffers.sort((job1, job2) => convertDecimal(job1.price) - convertDecimal(job2.price))
