...
const Select: React.FC<SelectComponent> & SelectOptions = ({ children, ...props }) => {
	...
	return (
		<div className={`${props.className}`}>
			<Box
				bg={useColorModeValue('snow.300', 'night.400')}
				className='flex justify-between items-center rounded py-2 px-4 cursor-pointer'
				onClick={() => setOpen((prev) => !prev)}
			>
				<div className='flex gap-2'>{selectedText}</div>
				<span className='ml-4'>{open ? <IoIosArrowUp /> : <IoIosArrowDown />}</span>
			</Box>
			{open && (
				<Box
					bg={useColorModeValue('snow.300', 'night.400')}
					className='absolute top-12 w-max max-h-64 rounded overflow-x-hidden overflow-y-auto shadow z-50'
				>
					{React.Children.map(children as React.ReactElement[], (child: React.ReactElement) => {
						return !child?.props?.disabled ? (
							<Select.Option
								onClick={(e: MouseEvent) => handleSelect(e, child?.props?.value, child?.props?.children)}
								{...child?.props}
							/>
						) : (
...
const Option: React.FC<OptionComponent> = ({ children, onClick, ...props }) => (
	<Box
		bg={props.disabled ? useColorModeValue('snow.100', 'night.200') : useColorModeValue('snow.300', 'night.400')}
