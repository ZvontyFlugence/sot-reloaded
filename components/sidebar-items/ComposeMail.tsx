...
const ComposeMail: React.FC<ComposeMailProps> = ({ isOpen, onClose, profile }) => {
	...
	useEffect(() => {
		if (profile) setUsers((curr) => [...curr, profile])
	}, [profile])
	...
	const handleSearch = (e: SyntheticEvent) => {
		...
		request({
			...
			if (data?.users) {
				setResults(
					data.users.filter((u: Participant) => {
						return (
	...
	const resultItem = (result: Participant) => (
		<div
			key={result.id}
			className='flex items-center gap-2 py-2 px-4 cursor-pointer rounded hover:bg-frost-100'
			...
			<AlertDialogContent bgColor='night.400' color='snow.100'>
				...
				<AlertDialogBody className='flex flex-col gap-2'>
					<div className='relative w-max'>
						<Input
							className='border border-snow-100 border-opacity-25 shadow-md rounded'
							type='text'
							value={participant}
					...
					<div className='my-2'>
						<span>To: </span>
						{users.map((u) => (
							<Tag key={u.id} size='lg' colorScheme='' color='snow.100' borderRadius='full'>
								<Avatar src={u.image} size='xs' name={u.username} ml={-1} mr={2} />
								<TagLabel color='snow.100'>{u.username}</TagLabel>
