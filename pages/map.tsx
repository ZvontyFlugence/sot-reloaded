...
import { Country, PrismaClient, Region, RegionGeom, RegionNeighbor, RegionPath } from '@prisma/client'
...
import { MAP_STYLE, RESOURCES } from '@/core/constants'
import { GMap } from 'primereact/gmap'
import { getResource, getResourceColor } from '@/core/uiHelpers/resources'

interface MapProps {
    gmapKey: string
    regions: RegionWithMapInfo[]
}

type RegionWithMapInfo = (Region & {
    core: Country
    owner: Country
    borders: (RegionGeom & {
        geom: RegionPath[]
...
const Map: React.FC<MapProps> = ({ gmapKey, regions }) => {
        ...
        <Layout>
            <div className='flex flex-col w-full'>
                ...
                <div className='my-2 flex justify-center md:justify-end gap-2'>
                    <Button size='sm' colorScheme='' bgColor='frost.300' color='snow.100' onClick={() => setMode('political')}>Political</Button>
                    <Button size='sm' colorScheme='' bgColor='aurora.green' color='snow.100' onClick={() => setMode('resources')}>Resources</Button>
                    <Button size='sm' colorScheme='' bgColor='aurora.red' color='snow.100' onClick={() => setMode('battles')}>Battles</Button>
                </div>
                <div className='h-[38rem] w-full'>
                    <GMap
                        overlays={overlays}
                        options={DEFAULT_MAP_PROPS}
                        style={{ width: 'auto', height: '38rem' }}
                    />
                </div>
            </div>
        </Layout>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    ...
    return {
        props: {
            gmapKey: process.env.NEXT_PUBLIC_GMAP_KEY,
