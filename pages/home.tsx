...
const Home: React.FC = () => {
    ...
    const { data } = useSWR('/api/me/job', jobFetcher)
    ...
    return user ? (
        <Layout>
            <div className='flex flex-col w-full'>
                <Heading color='aurora.red.500' className='text-2xl font-semibold'>My Home</Heading>
                        ...
                        <GridItem colSpan={2}>
                            <Card>
                                ...
                                <Card.Content className='text-white'>
                                    <p>Current Strength: {user.strength} <i className='sot-icon sot-str' /></p>
                                    <Button
                                        className='mt-2'
                                        variant='solid'
                                        colorScheme=''
                                ...
                                <Card.Content className='text-white'>
                                    {data?.job ? (
                                        <div className='flex justify-between items-center mt-2'>
                                            <div className='flex items-center gap-2 cursor-pointer' onClick={() => router.push(`/company/${data?.job.id}`)}>
                                                <Avatar src={data?.job.image} name={data?.job.name} />
                                                {data?.job.name}
                                                <i className='' title='' />
                                            </div>
                                            <Button
