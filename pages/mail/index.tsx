...
const Mail: React.FC = () => {
    ...
    const [mail, setMail] = useState<IMsgThread[]>([])
        ...
        <Layout>
            <div className='flex flex-col w-full'>
                ...
                <div className='flex justify-center md:justify-end gap-4 mt-2'>
                    <Button variant='solid' size='sm' colorScheme='' bgColor='aurora.green' color='snow.100' onClick={onOpen}>Compose</Button>
                    <Button variant='solid' size='sm' colorScheme='' bgColor='frost.300' color='snow.100' onClick={readAll}>Read All</Button>
                    <Button variant='solid' size='sm' colorScheme='' bgColor='aurora.red' color='snow.100' onClick={deleteAll}>Delete All</Button>
                </div>
                {mail.length > 0 ? (
                    <div className='mt-4 mx-0 md:mx-12 bg-night-400 shadow-md rounded'>
                        {mail.map((thread) => (
                            <MailItem key={thread.id} thread={thread} />
                        ))}
                    </div>
                ) : (
