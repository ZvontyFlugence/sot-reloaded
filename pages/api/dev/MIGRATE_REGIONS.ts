import { Region, PrismaClient, RegionPath, Prisma } from '.prisma/client'
    ...
    switch (req.method) {
        case 'POST': {
            ...
            await withPrisma(async (client: PrismaClient) => {
                ...
                created = await Promise.all(regions.map(async (r: MongoRegion): Promise<Region | null> => {
                            ...
                            (r.borders as MongoRegionPath[][]).map(async (geom) => {
                                await client.regionGeom.create({
                                    data: {
                                        ...
                                        geom: {
                                            create: [
                                                ...geom.map(path => ({ lat: path.lat, lng: path.lng }))
                                            ]
                                        }
                                    }
                                })
